...
export class DeviceInventoryComponent implements OnInit {
  ...
  exportDevicesInv(event) { event.preventDefault(); alert('exporting ...'); }

  importDevicesInv(event) { event.preventDefault(); alert('importing...'); }

  toggleDeviceInvCheckBoxes(event) {
    $('.deviceInvCheckBoxes').each(function() {
      if ($('#deviceInvHeadCheckBox').prop('checked'))
      {
  ...
  deleteDeviceInv(event, id) { event.preventDefault(); alert('do you confirm to delete row id:' + id); }

  deleteDevicesInv(event):void {
    event.preventDefault();
    if (($('.deviceInvCheckBoxes:checked').length) > 0)
    {
      var Ids = [];
      $('.deviceInvCheckBoxes:checked').each(function(i, e){
