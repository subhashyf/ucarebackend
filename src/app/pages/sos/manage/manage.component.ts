...
export class SOSManageComponent implements OnInit {
  ...
  toggleSOSCheckBoxes(event) {
    $('.sosCheckBoxes').each(function() {
      if ($('#sosHeadCheckBox').prop('checked'))
      {
  ...
  exportSOSs(event) { event.preventDefault(); alert('exporting ...'); }

  importSOSs(event) { event.preventDefault(); alert('importing...'); }

  deleteSOS(event, id) { event.preventDefault(); alert('do you confirm to delete row id:' + id); }

  deleteSOSs(event):void {
    event.preventDefault();
    if (($('.sosCheckBoxes:checked').length) > 0)
    {
      var Ids = [];
      $('.sosCheckBoxes:checked').each(function(i, e){
