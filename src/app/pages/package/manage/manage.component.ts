...
export class PackageManageComponent implements OnInit {
  ...
  togglePackageCheckBoxes(event) {
    $('.packageCheckBoxes').each(function() {
      if ($('#packageHeadCheckBox').prop('checked'))
      {
  ...
  exportPackages(event) { event.preventDefault(); alert('exporting ...'); }

  importPackages(event) { event.preventDefault(); alert('importing...'); }

  deletePackage(event, id) { event.preventDefault(); alert('do you confirm to delete row id:' + id); }

  deletePackages(event):void {
    event.preventDefault();
    if (($('.packageCheckBoxes:checked').length) > 0)
    {
      var Ids = [];
      $('.packageCheckBoxes:checked').each(function(i, e){
