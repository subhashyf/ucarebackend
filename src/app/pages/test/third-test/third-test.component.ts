...
export class ThirdTestComponent implements OnInit {
  ...
  toggleProviderCheckBoxes(event) {
    $('.providerCheckBoxes').each(function() {
      if ($('#providerHeadCheckBox').prop('checked'))
      {
  ...
  exportProviders(event) { event.preventDefault(); alert('exporting ...'); }

  importProviders(event) { event.preventDefault(); alert('importing...'); }

  deleteProvider(event, id) { event.preventDefault(); alert('do you confirm to delete row id:' + id); }

  deleteProviders(event):void {
    event.preventDefault();
    if (($('.providerCheckBoxes:checked').length) > 0)
    {
      var Ids = [];
      $('.providerCheckBoxes:checked').each(function(i, e){
