...
export class KitManageComponent implements OnInit {
  ...
  toggleKitCheckBoxes(event) {
    $('.kitCheckBoxes').each(function() {
      if ($('#kitHeadCheckBox').prop('checked'))
      {
  ...
  exportKits(event) { event.preventDefault(); alert('exporting ...'); }

  importKits(event) { event.preventDefault(); alert('importing...'); }

  deleteKit(event, id) { event.preventDefault(); alert('do you confirm to delete row id:' + id); }

  deleteKits(event):void {
    event.preventDefault();
    if (($('.kitCheckBoxes:checked').length) > 0)
    {
      var Ids = [];
      $('.kitCheckBoxes:checked').each(function(i, e){
