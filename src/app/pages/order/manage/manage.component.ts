...
export class OrderManageComponent implements OnInit {
  ...
  toggleOrderCheckBoxes(event) {
    $('.orderCheckBoxes').each(function() {
      if ($('#orderHeadCheckBox').prop('checked'))
      {
  ...
  exportOrders(event) { event.preventDefault(); alert('exporting ...'); }

  importOrders(event) { event.preventDefault(); alert('importing...'); }

  deleteOrder(event, id) { event.preventDefault(); alert('do you confirm to delete row id:' + id); }

  deleteOrders(event):void {
    event.preventDefault();
    if (($('.orderCheckBoxes:checked').length) > 0)
    {
      var Ids = [];
      $('.orderCheckBoxes:checked').each(function(i, e){
